...
\subsection{\label{expressions}expressions}
If at any place in the input file PVMOS encounters an expression between two suare brackets, i.e. [\textless expression\textgreater], the expression will be evaluated. PVMOS will then interpret the whole expression including the brackts as the result of the evaluation. For example, \texttt{[1 + 2 ]} will be interpreted as if it says \texttt{3}. The expresssions can include mathematical expressions and also variables. Variables can be defined with the  \texttt{define} command (see Section \ref{commref}). Note that expressions can be used throuhout the input file wherever a numerical value is required. Some examples:
\texttt{define N 10} \\
...
\texttt{define}	&  Define a variable. These variables can be used in expressions (see Section \ref{expressions}). The command takes two arguments:
\begin{enumerate}
