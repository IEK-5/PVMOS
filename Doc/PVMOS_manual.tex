...
As we have seen in the command reference, one can define variables in PVMOS. These variables can be accessed by placing the variable name between square brackets, i.e.  [\textless variable-name\textgreater]. You can use the variable value whereever PVMOS expects a numerical value. If PVMOS is compiled with libmatheval support it also features the ability to evaluate mathematical expressions placed between square brackets. Naturally you can use the defined variables in these expressions. At least for now the latter feature is not supported on windows systems (untill someone takes the time and effort to compile libmatheval for windows), i.e. under windows, or generally without libmathevel support, only variable names will be evaluated between brackets. Some examples:
\texttt{define N 10} \\
\texttt{newmesh 0 0 1 1 [N] [N] mesh}\\
\texttt{newmesh 0 0 1 1 [N+5] [N\textasciicircum 2] mesh2}\\
\texttt{define Jph 0.03} \\
\texttt{define Jph2 [Jph/2]} \\
\texttt{define kT 0.0259} \\
\texttt{define J0 1e-12} \\
\texttt{define Voc [kT*log(Jph/J0+1)]} \\

With libmatheval support the following functions are supported:
\begin{itemize}
