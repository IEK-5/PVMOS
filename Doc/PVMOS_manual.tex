...
\end{itemize}\\
\texttt{VVel(i\_el,i\_elec,V)}	& Estimate the applied voltage such that the potential in element \texttt{i\_el} in electrode \texttt{i\_elec} equals \texttt{V} V
\begin{itemize}
\item Input: Element index number \texttt{i\_el}, where $0 \le $\texttt{i\_el}$ < $\texttt{Nel}, electrode index number \texttt{i\_elec}, where $0 \le $\texttt{i\_elec}$ < $\texttt{Nelec}, and the potential $V$ 
