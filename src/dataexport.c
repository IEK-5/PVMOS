...
void PrintPars(char *fn, mesh *M)
{
	...
	for (i=0;i<M->Na;i++)
	{
		...
		for (k=0;k<M->Nel;k++)
		{
			...
			fprintf(f,"Rvp %3i: %14e\tRvn %3i: %14e\tRvg %3i: %14e\n", k, M->P[i].Rvp[k], k, M->P[i].Rvn[k], k, M->P[i].Rvg[k]);
			if (k>0)
			{
...
void BubbleSortIIIV(int n, double *V, double *I1, double *I2, double *I3, double *I4)
{
	int a, b, s;
	double c;
	for(a=0;a<(n-1);a++)
...
void PrintInIp(char *fn, mesh *M, int *selected)
{
	...
	Ig=malloc((M->res.Nva+1)*sizeof(double));
	for (i=0;i<M->res.Nva;i++)
	{
		V[i]=M->res.Va[i];
		I[i]=M->res.I[i];
		...
		{			
			...
			for(i=0;i<M->res.Nva;i++)
			{
				for(k=0;k<M->Nel;k++)
				{
					...
					if (M->P[N->P].Rvg[k]>0)
						Ig[i]+=Area*(M->P[N->P].Vg-M->res.Vn[i][k][N->id])/M->P[N->P].Rvg[k];
	...
	BubbleSortIIIV(M->res.Nva, V, In , Ip, Ig, I);
	
	for (i=0;i<M->res.Nva;i++)
		fprintf(f,"%e %e %e %e %e\n", V[i], Ip[i], In[i], Ig[i], I[i]);
	fclose(f);
	free(V);
	free(I);
	free(Ip);
